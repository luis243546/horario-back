...
public class PeriodController {
    ...
    // ✅ NUEVO: Obtener período activo
    // Este endpoint es crítico para el funcionamiento del panel de asistencia de docentes
    // Permite que el sistema cargue automáticamente el período académico actual
    // sin que el usuario docente tenga que seleccionarlo manualmente
    @GetMapping("/active")
    public ResponseEntity<ApiResponse<PeriodResponseDTO>> getActivePeriod() {
        return ResponseEntity.ok(
                ApiResponse.success(
                        periodService.getActivePeriod(),
                        "Período activo obtenido con éxito"
                )
        );
    }
